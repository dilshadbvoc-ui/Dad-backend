...
async function checkMetaLeadsAPI() {
    try {
                    ...
                    if (leads.length > 0) {
                        ...
                        for (const lead of leads) {
                            try {
                                ...
                                if (e.message?.includes('Duplicate') || e.message?.includes('Unique constraint')) {
                                    console.log(`      Lead ${lead.id} already exists in DB.`);
                                } else {
                                    console.log(`      Failed to sync lead ${lead.id}: ${e.message}`);
                                }
                            }
                        }
                    }
        ...
        console.log(`\nDone checking Meta. Total Meta Leads Found: ${found}. Newly Synced: ${processed}`);
    } catch (e: any) {
