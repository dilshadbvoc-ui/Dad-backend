...
_Powered by CRM Automation_`;
    ...
    /**
     * Fully self-contained: re-queries the same tables `reportController.ts`'s
     * `getDailyReport` does (its query pattern, not the function itself) so the
     * mobile app's `/reports/daily-report` contract stays completely untouched.
     * Backs the public WhatsApp-shared daily summary page — a business-owner-facing
     * "how did today go" view (calls + leads + revenue), not a raw call log.
     */
    static async getDailyBusinessSummary(organisationId: string, dateStr?: string) {
        const day = dateStr ? new Date(`${dateStr}T00:00:00`) : new Date();
        day.setHours(0, 0, 0, 0);
        const nextDay = new Date(day);
        nextDay.setDate(nextDay.getDate() + 1);

        const [org, users, newLeadsToday, wonDealsToday] = await Promise.all([
            prisma.organisation.findUnique({ where: { id: organisationId }, select: { name: true } }),
            prisma.user.findMany({
                where: { organisationId, isDeleted: false, isPlaceholder: false },
                select: { id: true, firstName: true, lastName: true, branch: { select: { name: true } } }
            }),
            prisma.lead.count({ where: { organisationId, isDeleted: false, createdAt: { gte: day, lt: nextDay } } }),
            prisma.opportunity.findMany({
                where: { organisationId, stage: 'closed_won', closeDate: { gte: day, lt: nextDay } },
                select: { ownerId: true, amount: true }
            })
        ]);
        ...
        let totalRevenue = 0;
        for (const deal of wonDealsToday) {
            totalRevenue += deal.amount || 0;
            if (deal.ownerId) revenueByOwner.set(deal.ownerId, (revenueByOwner.get(deal.ownerId) || 0) + (deal.amount || 0));
        }
        ...
        const perUser = await Promise.all(users.map(async (u) => {
            ...
            return {
                ...
                wonDeals,
                revenue: revenueByOwner.get(u.id) || 0
            };
        }));

        const rawCallsToday = await prisma.interaction.findMany({
            where: { organisationId, type: 'call', date: { gte: day, lt: nextDay }, isDeleted: false },
            select: { createdById: true, direction: true, phoneNumber: true, date: true, duration: true, callStatus: true }
        });
        const CALL_DEDUP_WINDOW_MS = 60 * 1000;
        const sortedCalls = [...rawCallsToday].sort((a, b) => a.date.getTime() - b.date.getTime());
        const callsToday: typeof rawCallsToday = [];
        ...
        return {
            ...
            dateLabel: day.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }),
            headline: {
