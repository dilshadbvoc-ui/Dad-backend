...
const updateLeadEnhanced = async (req, res) => {
    ...
    try {
        ...
        if (sanitizedData.phone && sanitizedData.phone !== existingLead.phone) {
            ...
            const duplicateCheck = await DuplicateLeadService.checkDuplicate(sanitizedData.phone, undefined, organisationId || '', existingLead.branchId || undefined);
            if (duplicateCheck.isDuplicate && duplicateCheck.existingLead && duplicateCheck.existingLead.id !== leadId) {
                logger_1.logger.warn('Attempted to update lead with duplicate phone', 'LEADS', userId, organisationId || undefined, { phone: sanitizedData.phone, leadId });
                return apiResponse_1.ResponseHandler.validationError(res, `A lead with this phone number already exists in this branch`);
            }
        }
        // 6. Update lead
