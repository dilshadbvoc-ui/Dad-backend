...
const getLeadWhatsAppMessages = async (req, res) => {
    try {
        ...
        if (lead.phone)
            buildPhoneVariants(lead.phone).forEach(v => phoneVariantSet.add(v));
        if (lead.secondaryPhone)
            buildPhoneVariants(lead.secondaryPhone).forEach(v => phoneVariantSet.add(v));
        const agentSelect = { select: { id: true, firstName: true, lastName: true } };
        // 1a. Fetch by leadId — uses WhatsAppMessage_leadId_idx (fast B-Tree scan)
        const byLeadId = await prisma_1.default.whatsAppMessage.findMany({
        ...
        const byPhone = phoneVariantSet.size > 0
            ? await prisma_1.default.whatsAppMessage.findMany({
                where: {
                    organisationId: orgId,
                    isDeleted: false,
